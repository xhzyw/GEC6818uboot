...
static int update_from_sd (char *part, char *dir, char *file)
{
	...
	if (file != NULL)
	{
		...
		offset = CFG_FASTBOOT_TRANSFER_BUFFER;
		count = 0;
		sprintf(tmp, "Reading %s", filename);
		fboot_lcd_status(tmp);
		size = file_fat_read (filename, (unsigned char *) offset, count);
...
int do_sdfuse(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if((argc == 2) && (strcmp(argv[1], "forceupdate") == 0))
		force_update_flag = 1;
	...
	s = getenv("ubuntucrc");
	ubuntucrc = s ? (int)simple_strtol(s, NULL, 10) : 0;
	
	crc = get_file_crc(0, "/GEC6818-android", "GECuboot.bin");
	if(crc != 0)
	{
		crc = get_file_crc(crc, "/GEC6818-android", "boot.img");
		if(crc != 0)
		{
			crc = get_file_crc(crc, "/GEC6818-android", "system.img");
			if(crc != 0)
			{
				if((androidcrc == 0) || (androidcrc != crc) || (force_update_flag == 1))
				{
					fboot_lcd_start();
					update_from_sd("GECuboot", "/GEC6818-android", "GECuboot.bin");
					update_from_sd("boot", "/GEC6818-android", "boot.img");
					update_from_sd("system", "/GEC6818-android", "system.img");
					update_from_sd("userdata", "/GEC6818-android", "userdata.img");
					update_from_sd("cache", "/GEC6818-android", "cache.img");
					update_from_sd("recovery", "/GEC6818-android", "recovery.img");
					...
					later_run_set_env("/GEC6818-android", "env.txt");
					fboot_lcd_stop();
	...
	crc = get_file_crc(0, "/GEC6818-qt", "GECuboot.bin");
	if(crc != 0)
	{
		crc = get_file_crc(crc, "/GEC6818-qt", "boot.img");
		if(crc != 0)
		{
			crc = get_file_crc(crc, "/GEC6818-qt", "qt-rootfs.img");
			if(crc != 0)
			{
				if((qtcrc == 0) || (qtcrc != crc) || (force_update_flag == 1))
				{
					fboot_lcd_start();
					update_from_sd("ubootpak", "/GEC6818-qt", "GECuboot.bin");
					update_from_sd("boot", "/GEC6818-qt", "boot.img");
					update_from_sd("system", "/GEC6818-qt", "qt-rootfs.img");
					...
					later_run_set_env("/GEC6818-qt", "env.txt");
					fboot_lcd_stop();
					do_reset(NULL, 0, 0, NULL);
					return 0;
				}
	...
	crc = get_file_crc(0, "/GEC6818-ubuntu", "GECuboot.bin");
	if(crc != 0)
	{
		crc = get_file_crc(crc, "/GEC6818-ubuntu", "boot.img");
		if(crc != 0)
		{
			crc = get_file_crc(crc, "/GEC6818-ubuntu", "ubuntu-rootfs.tar.bz2");
			if(crc != 0)
			{
				if((ubuntucrc == 0) || (ubuntucrc != crc) || (force_update_flag == 1))
				{
					fboot_lcd_start();
					
					update_from_sd("GECuboot", "/GEC6818-ubuntu", "GECuboot.bin");
					update_from_sd("boot", "/GEC6818-ubuntu", "boot.img");
				
					char tmp[32];
					sprintf(tmp, "%d", crc);
					setenv("ubuntucrc", tmp);
					saveenv();

					later_run_set_env("/GEC6818-ubuntu", "env.txt");

					char buf[256];
					strcpy(buf, getenv("bootargs"));
					strcat(buf, " command=burn-ubuntu console=tty0");
					setenv("bootargs", buf);

					run_command("ext4load mmc 2:1 0x48000000 uImage", 0);
					run_command("ext4load mmc 2:1 0x49000000 debug-ramdisk.img", 0);
					run_command("bootm 48000000", 0);
