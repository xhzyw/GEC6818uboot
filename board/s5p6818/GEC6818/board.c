...
#define	UPDATE_KEY		(PAD_GPIO_A + 28)
static void check_auto_update(int io)
{
	unsigned int grp = PAD_GET_GROUP(io);
	unsigned int bit = PAD_GET_BITNO(io);
	int level = 1, i = 0;
	char *cmd = "sdfuse forceupdate";
	int wait = 100;
	...
	if(i == wait)
	{
		printf("Update key pressed, Force update\n");
		run_command("sdfuse forceupdate", 0);
	}
	else
	{
